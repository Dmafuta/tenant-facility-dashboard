...
import type { DepartmentData } from '@/lib/api/departments'
import type { VendorData } from '@/lib/api/vendors'

const STAFF_TYPES = ['permanent_staff', 'casual_staff', 'outsourced']

async function serverFetch<T>(path: string, token?: string): Promise<T | undefined> {
  try {
...
async function loadStaff(token?: string): Promise<Person[] | undefined> {
  const all = await serverFetch<PersonData[]>('/people', token)
...
export default async function HRPage() {
  ...
  const [staff, departments, vendors] = await Promise.all([
    loadStaff(token),
    serverFetch<DepartmentData[]>('/departments', token),
    serverFetch<VendorData[]>('/vendors', token),
  ])
